...
def train(args, train_source_loader, train_target_loader, source_model, model, layer_head, discriminator, criterion, optimizer, optimizer_d, epoch, print_freq=10):
    ...
    for i,data in enumerate(zip(train_source_loader,train_target_loader)):
        ...
        ce_loss = criterion[0](source_output_seg, source_label_var) + 0.01*criterion[0](target_output_seg, fake_label)
        bce_loss = criterion[2](source_output_cls,source_cls_label) + 0.01*criterion[2](target_output_cls, fake_cls)
        dice_loss = criterion[1](source_output_seg, source_label_var) + 0.01*criterion[1](target_output_seg,fake_label)
...
def parse_args():
    ...
    parser.add_argument('--loss', help='change model', default='ce+bce+dice+0.01self_training', #self_entropy+clhpy+loss_backgroundce#entropy_pseudo_+[1, 20, 20, 20]+bce+hype+loss_backcls-+pseudo_seg_ce+pseudo_cls_bce_loss+pseudo_seg_ce+cls_bce
                        type=str)
    ...
    parser.add_argument('--describe', default='Adv_ST_3.21v2',#fixed_freeze_cls2final_1_freeze_cls2final_1_
