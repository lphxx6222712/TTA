...
#             seg_labels = data[1]
#             inputs = inputs.cuda()
#             seg_out, cls_out, cls_fea, seg_fea = model(inputs)
#
#             if start_test:
#                 all_seg_fea = seg_fea#.float().cpu()
#                 all_seg_out = seg_out#.float().cpu()
#                 all_seg_labels = seg_labels#.float().cpu()
#                 all_cls_fea = cls_fea#.float().cpu()
#                 all_cls_out = cls_out#.float().cpu()
#                 start_test = False
#             else:
#                 if all_seg_fea.shape[0]>=100: break
#                 all_seg_fea = torch.cat((all_seg_fea, seg_fea),0)#.float().cpu()), 0)
#                 all_seg_out = torch.cat((all_seg_out, seg_out),0)#.float().cpu()), 0)
#                 all_seg_labels = torch.cat((all_seg_labels, seg_labels),0)#.float()), 0)
#                 all_cls_fea = torch.cat((all_cls_fea, cls_fea),0)#.float().cpu()), 0)
#                 all_cls_out = torch.cat((all_cls_out, cls_out),0)#.float().cpu()), 0)
#
#     all_seg_out = torch.nn.Softmax(dim=1)(all_seg_out)
#     _, predict = torch.max(all_seg_out, 1)
#     dice_plain = compute_average_dice(predict.cpu().data.numpy().flatten(), all_seg_labels.cpu().data.numpy().flatten())
#     #accuracy = torch.sum(torch.squeeze(predict).float() == all_label).item() / float(all_label.size()[0])
#     all_seg_fea = torch.nn.BatchNorm2d(all_seg_fea.shape[1]).cuda()(all_seg_fea)
#     all_seg_fea = torch.cat((all_seg_fea, torch.ones(all_seg_fea.size(0), 1, all_seg_fea.size(2), all_seg_fea.size(3)).cuda()), 1) #?
#     #all_seg_fea_flatten = all_seg_fea.reshape(-1,17)
#     #L2_norm = torch.norm(all_seg_fea_flatten,p=2,dim=1)
#     #all_seg_fea_flatten = (all_seg_fea_flatten.t() / L2_norm).t()
#     # for j in range(all_seg_fea.size(0)):
#     #     div_results = torch.div(all_seg_fea[j], L2_norm[j])
#     #     all_seg_fea[j,:,:,:] = div_results
#
#     K = all_seg_out.size(1)
#     aff = all_seg_out.reshape(-1,all_seg_out.shape[1])
#     initc = torch.mm(aff.t(),all_seg_fea_flatten)
#     initc = initc / (1e-8 + aff.sum(axis=0)[:,None])
#     #initc = torch.mm(aff.reshape(-1,4).t(),all_seg_fea.reshape(-1,17))
#     #initc = initc / (1e-8 + aff.reshape(-1,4).sum(axis=0)[:, None])  # compute the classification center
#     dd = torch.zeros_like(all_seg_out).cpu().data.numpy()
#     for j in range(all_seg_fea.size(2)):
#         for k in range(all_seg_fea.size(3)):
#             dd[:,:,j,k] = cdist(all_seg_fea[:,:,j,k].cpu().data.numpy(), initc.cpu().data.numpy(), 'cosine')
#     pred_label = dd.argmin(axis=1)
#     dice_argmin =  compute_average_dice(pred_label.flatten(), all_seg_labels.cpu().data.numpy().flatten())
#
#     for round in range(1):
#         aff = torch.eye(K)[pred_label].cuda()
#         initc = torch.mm(aff.reshape(-1,4).t(),all_seg_fea_flatten.reshape(-1,17))
#         initc = initc / (1e-8 + aff.reshape(-1,4).sum(axis=0)[:, None])
#         dd = torch.zeros_like(all_seg_out).cpu().data.numpy()
#         for j in range(all_seg_fea.size(2)):
#             for k in range(all_seg_fea.size(3)):
#                 dd[:, :, j, k] = cdist(all_seg_fea[:, :, j, k].cpu().data.numpy(), initc.cpu().data.numpy(), 'cosine')
#         pred_label = dd.argmin(axis=1)
#         dice_eye =  compute_average_dice(pred_label.flatten(), all_seg_labels.cpu().data.numpy().flatten())
#
#     log_str = 'Dice = {:.2f}% -> {:.2f}%'.format(dice_plain[0] * 100, dice_eye[0] * 100)
...
def train(args, train_source_loader, train_target_loader, model, layer_head, discriminator, criterion, optimizer, optimizer_d, epoch, print_freq=10):
    ...
    #obtain_label(train_loader, model, args)
    model_source = torch.copy
    model.train()
    ...
    for i,data in enumerate(zip(train_source_loader,train_target_loader)):
        ...
        ce_loss = criterion[0](source_output_seg, source_label_var)
        ce_loss_target = criterion[0](target_output_seg, target_label_var)
        bce_loss = criterion[2](source_output_cls,source_cls_label)
        dice_loss = criterion[1](source_output_seg, source_label_var)
        ...
        loss_c = ce_loss + bce_loss + dice_loss + lay_loss + ce_loss_target # + loss_background +

        loss_c.backward(retain_graph=True) #source supervised loss

        optimizer_lay.step()


        ## adversial training to fool the discriminate
        #if torch.sum(source_pred) > 0 and torch.sum(target_pred)>0:
        d_out = F.softmax(target_output_seg,dim=1)
        d_out = prob_2_entropy(d_out)
        for k in range(args.number_classes):
            d_out[:, k:k + 1, :, :] = ((sim_lay) * d_out[:, k, :, :]).unsqueeze(1)
        d_out = Discriminator(d_out)
        loss_g = Discriminator_loss(d_out, 1)
        loss_g.backward()
        optimizer.step()
        ...
        Discriminator.train()
        source_output_seg = source_output_seg.detach()
        d_out = F.softmax(source_output_seg, dim=1)
        d_out = prob_2_entropy(d_out)
        for k in range(args.number_classes):
            d_out[:, k:k + 1, :, :] = ((1-sim_lay) * d_out[:, k, :, :]).unsqueeze(1)
        d_out = Discriminator(d_out)
        loss_d = Discriminator_loss(d_out, 1)
        #loss_d.backward()

        #train with target
        target_output_seg = target_output_seg.detach()
        d_out = F.softmax(target_output_seg, dim=1)
        d_out = prob_2_entropy(d_out)
        for k in range(args.number_classes):
            d_out[:, k:k + 1, :, :] = ((1-sim_lay) * d_out[:, k, :, :]).unsqueeze(1)
        d_out = Discriminator(d_out)
        loss_d = Discriminator_loss(d_out, 0)+loss_d
        loss_d.backward()
...
def train_seg(args, result_path, logger):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loss_c, loss_g, loss_d, dice_train, dice_1, dice_2, dice_3 = \
            train(args, train_source_loader, train_target_loader, net, layer_head, Discriminator, criterion, optimizer, optimizer_d, epoch)
...
def parse_args():
    ...
    parser.add_argument('--loss', help='change model', default='ce+bce+dice+OTSU_lay+self_training', #self_entropy+clhpy+loss_backgroundce#entropy_pseudo_+[1, 20, 20, 20]+bce+hype+loss_backcls-+pseudo_seg_ce+pseudo_cls_bce_loss+pseudo_seg_ce+cls_bce
                        type=str)
    ...
    parser.add_argument('--model-path', default='/home/isalab102/Documents/hxx/RETOUCH_SFUDA_new_split/result/train/unet_Cirrus_nopre_NLL+dice+bce_0.001_no_dataset_norm_11.08v1/checkpoint/model_best.pth.tar', type=str)
    parser.add_argument('--save-every-checkpoint', action='store_true')
    parser.add_argument('--describe', default='ADVENT_with_seperated_bottelnecked_layer_head_(sim_lay)*target_g_(1-sim_lay)*(target_source_d)_self_training_3.20v1',#fixed_freeze_cls2final_1_freeze_cls2final_1_
