...
def compute_score(predict, gt, forground=1):
    ...
    # print('overlap:',overlap)
    if (overlap > 0):
        return 2 * overlap / ((predict == forground).sum() + (gt == forground).sum()), overlap / (
                    predict == forground).sum(), overlap / (gt == forground).sum(), overlap / (
                           (predict == forground).sum() + (gt == forground).sum() - overlap)
...
def compute_segment_score(ret_segmentation, cubes):
    REA_segementation, SRF_segementation, PED_segementation = 0.0, 0.0, 0.0
    # REA_segementation, SRF_segementation = 0.0, 0.0
    n1, n2, n3 = 0, 0, 0
    # n1, n2= 0, 0
    ...
    # avg_segmentation = (REA_segementation + SRF_segementation) / 2
...
def compute_single_segment_score(ret_segmentation):
    ...
    if not math.isnan(ret_segmentation[3]):
        SRF_segementation += ret_segmentation[3]
        n3 += 1
    if n1 + n2 + n3 == 0:
        avg_segmentation = 0
    else:
        #avg_segmentation = (REA_segementation + SRF_segementation) / (n1 + n2)
...
def target_seg2target_cls(array):
    ...
    #class_num_all = torch.zeros(array.shape[0], 2)
    for i in range(array.shape[0]):
        ...
        # if label.sum() == 0:
        #     class_num = torch.tensor([0, 0])
        # elif label.sum() == 2:  # np.array([0,255]):
        #     class_num = torch.tensor([1, 0])
        # elif label.sum() == 3:  # np.array([0,191]):
        #     class_num = torch.tensor([0, 1])
        # elif label.sum() == 4:  # np.array([0,128]):
        #     class_num = torch.tensor([1, 1])
        if label.sum() == 0:
            class_num = torch.tensor([0, 0, 0])
        elif label.sum() == 2:  # np.array([0,255]):
