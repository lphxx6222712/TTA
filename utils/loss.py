...
sys.path.append(
    '/home/hxx/Documents/hxx_code/pytorch/AI-challenger-Retinal-Edema-Segmentation/bilateralfilter/build/lib.linux-x86_64-3.7')
...
def loss_builder(loss_type):
    weight_1 = torch.Tensor([1, 20, 20, 20])  # weighted CE
    #criterion_1 = nn.CrossEntropyLoss(weight_1).cuda()
    criterion_1 = nn.NLLLoss(weight_1).cuda()
    criterion_2 = DiceLoss().cuda()
    criterion_3 = nn.BCELoss().cuda()
    criterion_4 = focal_loss().cuda()
    criterion_5 = nn.NLLLoss().cuda()

    criterion = [criterion_1, criterion_2, criterion_3,criterion_4, criterion_5]
    return criterion
